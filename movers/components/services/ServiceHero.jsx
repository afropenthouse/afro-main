...
const ServiceHero = () => {
  ...
  return (
    <>
      <div className="relative w-full h-[300px] sm:h-[350px] md:h-[400px]">
        ...
        <div className="relative w-full max-w-[1440px] mx-auto px-6 sm:px-8 md:px-10 lg:px-20 h-full flex flex-col justify-center">
          ...
          <nav className="flex items-center gap-2 sm:gap-3 text-white text-sm sm:text-base">
            ...
            <span className="text-white">Moving Services</span>
          </nav>
      ...
      {/* Stats Section */}
      <motion.div 
        className="w-full max-w-[1440px] mx-auto px-6 sm:px-8 md:px-10 lg:px-20 py-12 bg-[#FFF9FA]"
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <motion.div key={index} className="flex flex-col" variants={statItemVariants}>
              <span className="text-gray-600 text-sm uppercase tracking-wider font-medium pb-2 border-b border-gray-300 mb-3">
                {stat.label}
              </span>
