...
export async function verifyAccessToken  (req:IExpressRequest,res:Response,next:NextFunction):Promise<Response | void>{
    ...
    if(!accessToken){
        return ResponseHandler.sendErrorResponse({res,error:"Invalid token supplied",status_code:"LOGIN_REDIRECT"})
    }
    try{
        ...
        if(!session){
            return ResponseHandler.sendErrorResponse({res,error:"session does not exist"})
        }
